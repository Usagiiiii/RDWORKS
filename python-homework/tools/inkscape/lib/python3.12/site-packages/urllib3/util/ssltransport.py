...
class SSLTransport:
    ...
    def makefile(
        ...
        *,
        ...
        """
        Python's httpclient uses makefile and buffered my_io when reading HTTP
        ...
        if buffering is None:
            buffering = -1
        if buffering < 0:
            buffering = my_io.DEFAULT_BUFFER_SIZE
        if buffering == 0:
            if not binary:
        ...
        if reading and writing:
            buffer = my_io.BufferedRWPair(raw, raw, buffering)  # type: ignore[assignment]
        elif reading:
            buffer = my_io.BufferedReader(raw, buffering)
        else:
            assert writing
            buffer = my_io.BufferedWriter(raw, buffering)
        if binary:
            return buffer
        text = my_io.TextIOWrapper(buffer, encoding, errors, newline)
