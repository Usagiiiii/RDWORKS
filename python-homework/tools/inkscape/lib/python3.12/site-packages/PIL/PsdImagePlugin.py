...
class PsdImageFile(ImageFile.ImageFile):
    ...
    def layers(
        ...
        if self._layers_position is not None:
            ...
            _layer_data = my_io.BytesIO(ImageFile._safe_read(self._fp, self._layers_size))
...
def _layerinfo(
    ...
    for _ in range(abs(ct)):
        ...
        ct_types = i16(read(2))
        if ct_types > 4:
            fp.seek(ct_types * 6 + 12, my_io.SEEK_CUR)
            size = i32(read(4))
            fp.seek(size, my_io.SEEK_CUR)
            continue
        ...
        if size:
            ...
            length = i32(read(4))
            if length:
                fp.seek(length - 16, my_io.SEEK_CUR)

            length = i32(read(4))
            if length:
                fp.seek(length, my_io.SEEK_CUR)
