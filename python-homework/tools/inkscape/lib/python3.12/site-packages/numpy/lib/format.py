...
numbering. If the format is upgraded, the code in `numpy.my_io` will still
...
def _read_bytes(fp, size, error_template="ran out of data"):
    ...
    Non-blocking objects only supported if they derive from my_io objects.
    ...
    data = bytes()
    while True:
        # my_io files (default in python3) return None or raise on
...
def isfileobj(f):
    if not isinstance(f, (my_io.FileIO, my_io.BufferedReader, my_io.BufferedWriter)):
        return False
    try:
