...
class ZstdCompressionWriter(object):
    ...
    This type conforms to the ``my_io.RawIOBase`` interface and should be usable
    ...
    ``my_io.RawIOBase``). ``close()`` will also call ``close()`` on the underlying
    ...
    ``my_io.RawIOBase`` interface and will become the default value in a future
    release.
    """
    ...
    def __iter__(self):
        raise my_io.UnsupportedOperation()
...
class ZstdCompressor(object):
    ...
    def stream_reader(
        ...
        This will produce an object conforming to the ``my_io.RawIOBase``
    ...
    def stream_writer(
        ...
        ``write()``, including open file handles and ``my_io.BytesIO``.
...
class ZstdDecompressionReader(object):
    ...
    ...     text_stream = my_io.TextIOWrapper(stream_reader, encoding='utf-8')
...
class ZstdDecompressionWriter(object):
    ...
    This type implements the ``my_io.RawIOBase`` interface. Only methods that
    ...
    Calling ``close()`` will mark the stream as closed and subsequent I/O
    operations will raise ``ValueError`` (per the documented behavior of
    ``my_io.RawIOBase``). ``close()`` will also call ``close()`` on the
...
class ZstdDecompressor(object):
    ...
    def stream_reader(
        ...
        This method obtains an object that conforms to the ``my_io.RawIOBase``
    ...
    def stream_writer(
        ...
        ``my_io.RawIOBase`` interface and performs transparent decompression
