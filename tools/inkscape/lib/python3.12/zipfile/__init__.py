...
class ZipExtFile(my_io.BufferedIOBase):
    ...
    def readline(self, limit=-1):
        ...
        return my_io.BufferedIOBase.readline(self, limit)
    ...
    def seek(self, offset, whence=os.SEEK_SET):
        ...
        if not self._seekable:
            raise my_io.UnsupportedOperation("underlying stream is not seekable")
        curr_pos = self.tell()
...
class _ZipWriteFile(my_io.BufferedIOBase):
...
class ZipFile:
    ...
    def __init__(self, file, mode="r", compression=ZIP_STORED, allowZip64=True,
        ...
        if isinstance(file, str):
            ...
            while True:
                try:
                    self.fp = my_io.open(file, filemode)
                except OSError:
